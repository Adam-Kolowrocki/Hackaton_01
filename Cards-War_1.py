...
while len(player_1_cards) != 0 and len(player_2_cards) != 0:
    ...
    if player_1_cards[0] == player_2_cards[0]:
        player_1_cards.append(player_1_cards[0])
        player_2_cards.append(player_2_cards[0])
        player_1_cards.pop(0)
        player_2_cards.pop(0)
        print(f'Runda remisowa, gramy dalej')
    elif player_1_cards[0] > player_2_cards[0]:
        player_1_cards.append(player_1_cards[0])
        player_1_cards.append(player_2_cards[0])
        player_1_cards.pop(0)
        player_2_cards.pop(0)
        print(f'{player_1} wygrywa rundę i zabiera karty.')
    else:
        player_2_cards.append(player_2_cards[0])
        player_2_cards.append(player_1_cards[0])
        player_1_cards.pop(0)
        player_2_cards.pop(0)
        print(f'{player_2} wygrywa rundę i zabiera karty.')
    input('Naciśnij Enter...')
    print((clear))

if len(player_1_cards) == 0:
    print(f'Grę wygrywa {player_2} zbierając {len(player_2_cards)} kard.')
else:
    print(f'Grę wygrywa {player_1} zbierając {len(player_1_cards)} kard.')
